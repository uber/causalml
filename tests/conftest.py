...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--runjax", action="store_true", default=False, help="run jax tests"
    )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "jax: mark test as jax to run")
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_jax = False if config.getoption("--runjax") else True
    ...
    for item in items:
        ...
        if "jax" in item.keywords and skip_jax:
            item.add_marker(pytest.mark.skip(reason="need --runjax option to run"))
