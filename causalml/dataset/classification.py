...
def make_uplift_classification(n_samples = 1000,
                    ...
                    Notes
                    -----
                    The algorithm for generating the base classification dataset is adapted from the make_classification method in the sklearn package, that uses the algorithm in Guyon [1] designed to generate
                    the "Madelon" dataset.
                    References
                    ----------
                    ...
                    for xi in range(n_classification_features - n_classification_informative - n_classification_redundant - n_classification_repeated):
                    ...
                    for treatment_key_i in treatment_name:
                        ...
                        if treatment_key_i in n_uplift_increase_dict and n_uplift_increase_dict[treatment_key_i]>0:
                            ...
                            X_increase, Y_increase = make_classification(n_samples=n_all, n_features=n_uplift_increase_dict[treatment_key_i], 
                                                                         n_informative=n_uplift_increase_dict[treatment_key_i], n_redundant=0, 
                            ...
                            if n_uplift_increase_mix_informative_dict[treatment_key_i]>0:
                                for xi in range(n_uplift_increase_mix_informative_dict[treatment_key_i]):
                                    ...
                                    x_name.append(x_name_i)
                                    df_res[x_name_i] = np.random.uniform(-1,1) * df_res[np.random.choice(x_informative_name)] + np.random.uniform(-1,1) * df_res[np.random.choice(x_uplift_increase_name)]
                    ...
                    for treatment_key_i in treatment_name:
                        ...
                        if treatment_key_i in n_uplift_decrease_dict and n_uplift_decrease_dict[treatment_key_i]>0:
                            ...
                            X_decrease, Y_decrease = make_classification(n_samples=n_all, n_features=n_uplift_decrease_dict[treatment_key_i], 
                                                                         n_informative=n_uplift_decrease_dict[treatment_key_i], n_redundant=0, 
                                                                         n_clusters_per_class=1,
                                                                         weights=[1-adjust_class_proportion, adjust_class_proportion])
