...
class SynthDataGenerator:
    ...
    maxdepths: List[int] = [1, 2, 3], lambdas: List[float] = np.logspace(-5, 1, num = 5).tolist(), M: int = 30, early_stopping_rounds: int = 3, 
    ...
    def fit(self, X: pd.DataFrame, w: pd.Series, y: pd.Series, initial_taus: Optional[List[float]] = None):
        self.X = X
        self.y = y
        self.w = w
        np.random.seed(42)
        if initial_taus is None:
            ...
            initial_taus = [float(raw_tau), float(lm_tau), float(x_learner_tau), float(t_learner_tau)]
        else:
        ...
        taus = np.linspace(initial_taus_median - self.gamma * initial_taus_range,
                        initial_taus_median + self.gamma * initial_taus_range, self.Q)
        self.taus = taus
        self.dgps = []
        for real_tau in taus:
            self.dgps.append(miu_cv(
                y=np.asarray(self.y),
                w=np.asarray(self.w),
                X=self.X,
                real_tau=real_tau,
    ...
    **tree_args
    ...
    if miu_m_minus_1 is None:
        ...
        if not binary_y:
            ...
            constraints = ({
                'type': 'eq',
                'fun': lambda x: x[1] - x[0] - real_tau
            })
            res = minimize(fun = continuous_objective, x0 = x0, args = (Q, a, d), constraints = constraints, method = 'SLSQP')
        else:
            constraints = ({
                'type': 'eq',
                'fun': lambda x: logistic(x[1]) - logistic(x[0]) - real_tau
            })
            res = minimize(fun = binary_objective, x0 = x0, args = (w, y), constraints = constraints, method = 'SLSQP')
        ...
        # Group sizes and residuals
            resid0 = pd.Series(resid).groupby(R0).agg(['count', 'sum']).reset_index().rename(columns = {'index':'leaf_node'})
            resid1 = pd.Series(resid).groupby(R1).agg(['count', 'sum']).reset_index().rename(columns = {'index':'leaf_node'})
            num_params = len(resid0) + len(resid1)
            Q = np.diag(np.concatenate([resid0['count'].to_numpy(), resid1['count'].to_numpy()]) * lambda_)
            a = -2 * np.concatenate([
                resid0['sum'].to_numpy(),
                resid1['sum'].to_numpy()
            ])
            ...
            if val_X is not None and val_y is not None and val_w is not None:
                ...
                resid0 = pd.Series(R0).groupby(R0).agg(['count']).reset_index().rename(columns = {'index':'leaf_node'})
                resid1 = pd.Series(R1).groupby(R1).agg(['count']).reset_index().rename(columns = {'index':'leaf_node'})
            ...
            res = minimize(continuous_objective, x0, args=(Q, a, d), constraints=constraints, method='SLSQP')
        else:
            ...
            def con_m(x, R0_constraint, R1_constraint, prev0, prev1):
                ...
                for i in range(len(x)):
                    if i < ctrl_nodes:
                        ...
                        group_sum.append((logistic(prev0[idx] + x[i])).sum()/len(R0_constraint) * -1)
                    else:
                        idx = (R1_constraint == r1[i - ctrl_nodes])
                        group_sum.append((logistic(prev1[idx] + x[i])).sum()/len(R0_constraint))
            ...
            res = minimize(fun = binary_objective_m, x0 = x0, args = (y, w, R0, R1, lambda_), constraints = constraints, method = 'SLSQP')
        ...
        def miu_0m(x):
            ...
            return prev + leaves.merge(resid0, on = 'leaf_node', how = 'left')['leaf_value'].to_numpy()
        def miu_1m(x):
            prev = miu_1_m_minus_1(x)
            leaves = pd.DataFrame({'leaf_node': b_1m.apply(x)})
            return prev + leaves.merge(resid1, on = 'leaf_node', how = 'left')['leaf_value'].to_numpy()
...
def miu(
    ...
    **tree_args
    ...
    for i in range(M):
        if i == 0:
            ans = miu_m(
                y=y, w=w, X=X, real_tau=real_tau,
                val_X=val_X, val_y=val_y, val_w=val_w,
                max_depth=max_depth, lambda_=lambda_, **tree_args
            )
            best_model = ans
        else:
            ans = miu_m(
                y=y, w=w, X=X, miu_m_minus_1=ans,
                val_X=val_X, val_y=val_y, val_w=val_w,
                max_depth=max_depth, lambda_=lambda_, **tree_args
            )
        ...
        if np.nanargmin(loss) != best_model_ind:
            ...
            if verbose:
                print(f"Best tree: {best_model_ind + 1}, best tree loss: {loss[best_model_ind]}")
            return {
...
def miu_cv(
    ...
    lambdas: List[float] = np.logspace(-5, 1, num = 5).tolist(), # range of lambdas is like in glmnet
    M: int = 30,
    early_stopping_rounds: Union[int, float] = float('inf'),
    verbose: bool = False,
    n_jobs: int = -1,
    ...
    **tree_args
    ...
    if n_jobs > 1:
        with mp.Pool(processes=n_jobs) as pool:
            params_loss = pool.map(partial(miu_row, y=y, w=w, X=X, real_tau=real_tau, 
                                      train_frac=train_frac, val_frac=val_frac, M=M, 
                                      early_stopping_rounds=early_stopping_rounds, 
                                      verbose=False, **tree_args), 
                              [row for _, row in params_loss.iterrows()])
    else:
        params_loss = [miu_row(row, y=y, w=w, X=X, real_tau=real_tau, 
                          train_frac=train_frac, val_frac=val_frac, M=M, 
                          early_stopping_rounds=early_stopping_rounds, 
                          verbose=False, **tree_args) 
    ...
    final_model = miu(
        y=y, w=w, X=X, real_tau=real_tau,
        val_y = None, val_w = None, val_X = None,
...
def miu_row(
    ...
    **tree_args
    ...
    if val_frac > 0:
        ...
        val_indices_0 = np.random.choice(train_indices_0, size=val_size_0, replace=False)
        val_indices_1 = np.random.choice(train_indices_1, size=val_size_1, replace=False)
    ...
    pred = np.where(test_w == 1,
                    miu_row['best_model'][1](test_X),
                    miu_row['best_model'][0](test_X))
    ...
    if not binary_y:    
        ...
        baseline_pred = np.where(test_w == 1, logit(np.mean(test_y[test_w == 1])), logit(np.mean(test_y[test_w == 0])))
