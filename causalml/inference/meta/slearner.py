...
class LRSRegressor(BaseSRegressor):
    ...
    def estimate_ate(self, X, treatment, y):
        ...
        te = np.array([self.models[group].coefficients[0] for group in self.t_groups])
        te_lb = np.array([self.models[group].conf_ints[0, 0] for group in self.t_groups])
        te_ub = np.array([self.models[group].conf_ints[0, 1] for group in self.t_groups])
