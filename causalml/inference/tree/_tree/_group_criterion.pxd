...
# Treatment-group-aware regression criterion base shared by the causal- and
# uplift-tree criteria. Both families track per-group (control vs. treatment)
# sufficient statistics; this module is the single home for that machinery so
# the uplift criterion depends on the shared ``_tree`` kernel rather than on the
# sibling ``causal`` package.
...
from ._typedefs cimport int32_t, int64_t, intp_t, float64_t
from ._criterion cimport RegressionCriterion


cdef int32_t CONTROL_GROUP_IDX
