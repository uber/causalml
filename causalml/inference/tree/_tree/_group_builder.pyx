...
"""Shared tree builders for treatment-group trees (causal + uplift).

Both the causal and uplift trees grow a scikit-learn ``Tree`` with the stock
``DepthFirstTreeBuilder`` / ``BestFirstTreeBuilder`` traversal, but each needs a
small family-specific tweak to the loop. Rather than fork the ~570-line builder
twice, the common skeleton lives here and the differences are expressed as four
overridable ``cdef`` hooks (all no-ops by default, so this base reproduces the
stock traversal):

* ``_before_build(tree)`` -- once, after ``splitter.init``.
* ``_before_node_split(splitter, parent)`` -- after ``node_reset``, before the
  split search. ``parent`` is the parent node id (``_TREE_UNDEFINED`` at the
  root). Uplift threads the parent's regularized summary into the criterion here.
* ``_after_node_value(splitter, node_id)`` -- after ``node_value``. Uplift records
  this node's regularized summary for its children to shrink toward.
* ``_node_is_leaf_extra(splitter)`` -- extra leaf predicate OR-ed into the stock
  stopping rules. Causal uses it for its per-group minimum-size checks.
...
cdef class GroupDepthFirstTreeBuilder(TreeBuilder):
    """Build a treatment-group decision tree in depth-first fashion.

    DepthFirstTreeBuilder with overridable hooks for the causal / uplift trees.
    """

    # --- default hooks (no-ops): subclasses override as needed ---------------
    ...
    cpdef build(self, Tree tree, object X,
        ...
        with nogil:
            ...
            while not builder_stack.empty():
                ...
                # Family hook: e.g. thread the parent's regularized summary into
                # the criterion before the split search.
                self._before_node_split(splitter, parent)

                is_leaf = (depth >= max_depth or
                           n_node_samples < min_samples_split or
                           n_node_samples < 2 * min_samples_leaf or
                           weighted_n_node_samples < 2 * min_weight_leaf or
                           self._node_is_leaf_extra(splitter))
                ...
                # Family hook: e.g. record this node's regularized summary for its
                # children to shrink toward.
                self._after_node_value(splitter, node_id)
...
cdef class GroupBestFirstTreeBuilder(TreeBuilder):
    """Build a treatment-group decision tree in best-first fashion.

    The best node to expand is the frontier node with the highest split gain.
    BestFirstTreeBuilder with overridable hooks for the causal / uplift trees.
    """
    ...
    cdef inline int _add_split_node(
        ...
        # Family hook: thread the parent's summary into the criterion before the
        # split search (the root has no parent).
        self._before_node_split(
            splitter,
        ...
        is_leaf = (depth >= self.max_depth or
                   ...
                   parent_record.impurity <= EPSILON or
                   self._node_is_leaf_extra(splitter)
                   )
        ...
        # Family hook: record this node's summary for its children.
        self._after_node_value(splitter, node_id)
